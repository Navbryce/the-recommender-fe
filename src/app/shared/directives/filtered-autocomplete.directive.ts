...
export class FilteredAutocompleteDirective
  ...
  ngAfterContentInit(): void {
    this.input.ngControl.control.valueChanges.subscribe((newValue: string) =>
      this.inputUpdated(newValue)
    );
  }
  ...
  private inputUpdated(newValue: string) {
    const filteredOptions = this.normalizedOptions
      .filter((normalizedOption) =>
        normalizedOption.normalizedText.includes(newValue.toLowerCase())
      )
